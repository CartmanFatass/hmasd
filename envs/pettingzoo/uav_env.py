...
class MultiUAVEnv(AECEnv):
    """
    多无人机基站环境的基类
    
    实现了PettingZoo的AEC接口，提供了基本的无人机和用户模型
    """
    metadata = {
    ...
    def reset(self, seed=None, options=None):
        ...
        # 转换为HMASD所需的格式
        state_array = state
        obs_array = np.array([observations[agent]["obs"] for agent in self.agents])
    ...
    def step(self, actions):
        ...
        if self.terminations[self.agent_selection] or self.truncations[self.agent_selection]:
            self._was_dead_step(action=None)
            return
        
        # 更新所有无人机位置
        for i, agent in enumerate(self.agents):
            # 将归一化动作转换为实际速度
            velocity = actions[i] * self.max_speed
            
            # 更新位置
            new_position = self.uav_positions[i] + velocity * self.time_step
        ...
        # 更新智能体状态
        for agent in self.agents:
            self.rewards[agent] = reward / self.n_uavs  # 平均分配奖励
        ...
        next_observations = {}
        for agent in self.agents:
            next_observations[agent] = self._get_observation(agent)
        
        # 转换为HMASD所需的格式
        next_state_array = next_state
        next_obs_array = np.array([next_observations[agent]["obs"] for agent in self.agents])
        ...
        return next_state_array, next_obs_array, reward, done, info
    ...
    def _compute_path_loss(self, uav_pos, user_pos):
        ...
        distance_2d = np.sqrt(np.sum((uav_pos[:2] - user_pos) ** 2))
        height = uav_pos[2]
